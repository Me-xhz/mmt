<template>
	<view>
		...
		<view class="product container" :style="(iphoneXBottom=='68rpx'?'padding-bottom:150rpx':'')">
			...
			<view class="cont">
				<swiper :indicator-color="indicatorColor" :circular="circular" :indicator-active-color="indicatorActiveColor"
				 :indicator-dots="indicatorDots" :autoplay="autoplay" :interval="interval" :duration="duration">
					<view v-for="(item, index) in bannerItem" :key="index">
						<!-- {{item.msrc}} -->

						<swiper-item class="dufiltImage">
							<!-- <image @tap="setPreviewImage"  @load="swiperImageLoad" :src="item.msrc" :class="'slide-image ' + (swiperImgBl == true ? 'loaded' : '')"></image> -->
							<image :src="item.msrc" mode="scaleToFill" class="slide-image2"></image>
						</swiper-item>
					</view>
				</swiper>
			...
			<view class="team-bar">
				...
				<view class="team-price row">
					...
					<view class="team-price-n">¥{{itemData.market_price}}</view>
				</view>
				...
				<view class="to_share row team-share" @tap="shareTo" v-if="teamLabel.status==0">
			...
			<view class="product-info">
				<view class="info-team-header" v-if="teamLabel.max_team_limit && teamLabel.count_type!=2">
					<view class="info-team-header-box">
						<view v-if="teamLabel.time_type==0">
							<view class="info-team-number-n" v-if="teamLabel.count_type==0">
								...
								<text class="i">{{teamLabel.follow_limit}}</text>次，开团
								<text class="i">{{teamLabel.found_limit}}</text>次
							</view>
							<view class="info-team-number-n" v-if="teamLabel.count_type==1">
								<text class="now-use-title">当前可用：</text>拼团
								<text class="i">{{teamLabel.total_team_limit}}</text>次
							</view>
						</view>
				...
				<!-- 拼团参加列表 -->
				<view class="join-team" v-if="teamList.length>0">
			...
			<view class="product-nums bor-b-16" @tap="shippingBonus.length > 0?'setModalBonus':'goCouponList'">
				<view class="row ">
					...
					<view class="col nums-item-right">
						...
						<i class="team-coupon-text" v-if="shippingBonus.length > 0">领取优惠券</i>
						<i class="team-coupon-text team-coupon-text-none" v-if="shippingBonus.length ==0">去领券中心</i>
						<view class="iconfont icon-btn_xiangyoudajiantoux icon-right-arrow"></view>
					</view>
				</view>
			</view>
			...
			<view class="intro">
				...
				<view class="wxParse">
					<!-- <component :wxParseData="introContent.nodes" :is="wxParse"></component> -->
					<wxParse :wxParseData="introContent.nodes"></wxParse>
				</view>
			</view>
		...
		<view :animation="animationData" style="opacity:1;z-index:2000}};" :class="'drawer_attr_box ' + (iphoneXBottom=='68rpx'?'bt-68':'')"
		 v-if="showModalStatus">
			...
			<view class="drawer_attr_content">
				...
				<button class="buy_now_btn" @click.stop="team_type==0?'nowPurchase':'delegation'">确定</button>
			</view>
		</view>
		<!-- 领取红包 -->
		...
		<view class="footfixed ">
			<view class="left">
				...
				<view class="submit-icon" @tap="goRegister">
					...
					<button open-type="contact" class="icon-contact" session-from="拼团商品" :send-message-title="itemData.goods_name"
					 :send-message-path="'/packageA/team-list/team-list?teamId=' + itemData.goods_id" :send-message-img="itemData.gallery[0].msrc"
			...
			<view>
				<view class="bottom-buttons team-buttons row" v-if="canPurchase">
					...
					<!-- 活动已结束 -->
					<button v-if="teamLabel.status == 0" class="submit-button end-remind team-remind-big" disabled="true">活动已结束</button>
				</view>
				<!-- 商品已抢光 -->
				<button type="primary" style="background-color:#aaaaaa;color:#303030;opacity:1" v-else class="submit-button-big width-534"
...
<script>
	...
	import WxParse from '@/wxParse/wxParse.vue'
...
<style>
	...
	.footfixed .submit-icon .icon-ico {
		width: 54rpx;
		height: 54rpx;
	}

	.footfixed .submit-icon .icon-contact {
		position: absolute;
		width: 100%;
		height: 100%;
		top: 0;
	...
	.footfixed .submit-icon .text {
		font-size: 20rpx;
		color: #000;
	}

	.footfixed .submit-icon .cart-badge {
		position: absolute;
		top: 5rpx;
		right: 5rpx;
	...
	.footfixed .submit-icon .max-length {
		width: 50rpx;
		border-radius: 20rpx;
	}

	.footfixed .bottom-buttons {
		width: 436rpx;
	}

	.footfixed .bottom-buttons.team-buttons {
		width: 534rpx;
	}

	.footfixed .bottom-buttons .submit-button {
		width: 218rpx;
		height: 96rpx;
		line-height: 96rpx;
		border-radius: 0;
	...
	.footfixed .bottom-buttons.team-buttons .submit-button {
		width: 200rpx;
	}

	.footfixed .bottom-buttons .team-button {
		line-height: 1.5;
	}

	.footfixed .bottom-buttons.team-buttons .team-button-big {
		width: 334rpx;
		line-height: 1.5;
	}

	.footfixed .bottom-buttons .team-button .team-button-price {
		font-weight: 500;
		font-size: 36rpx;
	}

	.footfixed .bottom-buttons .team-button .team-button-price .i {
		font-size: 24rpx;
		margin-right: 5rpx;
	}

	.footfixed .bottom-buttons .team-button .team-button-text {
		font-size: 24rpx;
	}

	.footfixed .bottom-buttons .submit-button.add-cart {
		background-color: #303030;
		color: #fff;
	}

	.footfixed .bottom-buttons .submit-button.now-purchase {
	...
	.footfixed .submit-button-big-team {
		width: 534rpx !important;
	}

	/* 租机底部按钮样式开始 */

	.footfixed .submit-button[disabled] {
		background-color: #eb3c39;
		color: #fff;
		opacity: 0.3;
	}

	.footfixed .rent-button {
		display: flex;
		width: 436rpx;
		height: 96rpx;
		line-height: 96rpx;
	...
	.footfixed .rent-button .rent-btn {
		flex: 1;
		color: #555;
		font-size: 24rpx;
	}
	...
	.dufiltImage {
		/* background-image: url("http://hys-shop.oss-cn-hangzhou.aliyuncs.com/wx_icons/img_dufilt_swiper.png"); */
	...
	.ac_bar .date.color {
		font-size: 24rpx;
		color: #FFFFFF;
		background: rgba(230, 150, 36, 0.6);
		border-radius: 300px 0px 600px 300px;
	}

	.ac_bar .time-start .date-icon {
		position: absolute;
		right: 24rpx;
		top: 50%;
	...
	.ac_bar .time-end .time-date .span {
		background: rgba(137, 27, 0, 1);
		border-radius: 2px;
		color: #fff;
		font-size: 28rpx;
		width: 36rpx;
	...
	.ac_bar .text .price .price-icon {
		font-size: 24rpx;
		line-height: 50rpx;
		color: #fff;
	}

	.ac_bar .text .price .price-start {
		font-size: 84rpx;
		line-height: 84rpx;
		color: #fff;
	}

	.ac_bar .text .price .price-end {
		font-size: 36rpx;
		line-height: 50rpx;
		color: #fff;
	}
	...
	.product-info .info-attr-nums image {
		width: 40rpx;
		height: 40rpx;
	}

	.product-info .info-attr-nums .share_txt {
		font-size: 20rpx;
		margin-top: 5rpx;
	}

	.product-info .info-team-header {
		background-color: #fff;
		padding: 16rpx 0 0 26rpx;
	}

	.product-info .info-team-header .info-team-header-box {
		background-color: #fff2f2;
		width: 700rpx;
		padding: 6rpx 0;
		border-radius: 4rpx 200rpx 200rpx 200rpx;
		text-align: center;
	}

	.product-info .info-team-header .info-team-number-i {
		font-size: 28rpx;
		color: #303030;
		text-align: center;
		font-weight: 500;
	...
	.product-info .info-team-number-i .i {
		font-size: 32rpx;
		color: #eb3c39;
	}

	.product-info .info-team-header .info-team-number-n {
		font-size: 28rpx;
		color: #303030;
		text-align: center;
		/* margin-top: 10rpx; */
	...
	.product-info .info-team-number-n .i {
		font-size: 24rpx;
		color: #eb3c39;
		font-weight: 500;
	}
	...
	.product-info .info-name .discount_label {
	...
	.product-info .info-name .info-name_menu {
		color: #ff4222;
		text-align: center;
	}
	...
	.product-info .info-price .info-tab {
		display: inline-block;
		width: 60rpx;
		height: 30rpx;
		line-height: 32rpx;
	...
	.product-info .info-price .is-vip_price {
		color: #f23030;
		font-size: 48rpx;
		line-height: 48rpx;
		height: 48rpx;
	...
	.product-info .info-price .is-vip_price .icon_rmb {
		height: 48rpx;
		line-height: 58rpx;
	}

	/* .product-info .info-price .is-vip_price i {
  font-size: 24rpx;
  line-height: 24rpx;
  margin-right: 4rpx;
}

.product-info .info-price .is-vip_price i.s {
  margin: 0;
  color: #bbb;
  font-weight: normal;
} */

	/* .product-info .info-price .is-vip_price_txt image {
  width: 120rpx;
  height: 28rpx;
} */

	.product-info .info-price .is-vip_price .del {
		color: #979797;
		font-weight: normal;
		text-decoration: line-through;
	}

	.product-info .info-price .is-vip_price .del_text {
		margin-left: 14rpx !important;
	}

	.product-info .info-price .is-vip_lot_price {
		font-size: 26rpx;
		line-height: 26rpx;
		padding-top: 22rpx;
		margin-top: -6rpx;
	...
	.product-info .info-price .is-vip_lot_price .del {
		font-size: 28rpx;
		line-height: 28rpx;
		margin-left: 4rpx;
	}
	...
	.product-info .info-price .is-novip .price-shop {
		color: #030303;
	}

	.product-info .info-price .price-shop i {
		font-size: 26rpx;
		line-height: 26rpx;
		margin-right: 2rpx;
	}

	.product-info .info-price .price-shop i.s {
		margin-right: 16rpx;
		font-size: 26rpx;
		color: #767676;
		font-weight: normal;
	...
	.product-info .info-price .price-svip-silt {
		padding-top: 10rpx;
	}

	.product-info .info-price .price-svip-silt span {
		font-size: 26rpx;
		line-height: 38rpx;
		color: #767676;
	}

	.product-info .info-price .price-svip-silt .svip_price_shop {
		font-size: 28rpx;
		line-height: 38rpx;
		color: #f23030;
		padding-left: 16rpx;
	...
	.product-info .info-price .price-svip-silt .go-svip-lot {
		line-height: 1;
		padding: 8rpx 16rpx;
		text-align: center;
		min-width: 270rpx;
	...
	.product-info .info-price .go-svip-lot .left_log_bin {
		width: 0;
		height: 0;
		position: absolute;
		left: -8rpx;
		...
		border-right: 12rpx solid #fcee81;
		border-bottom: 12rpx solid transparent;
	}

	.product-info .info-price .go-svip-lot image {
		width: 8rpx;
		height: 14rpx;
		margin-left: 4rpx;
		margin-bottom: 4rpx;
	...
	.info-price .is-vip-ory-price {
		font-size: 26rpx;
		color: #767676;
	}

	.info-price .is-vip-ory-price .ory-price {
		font-size: 28rpx;
		line-height: 28rpx;
		margin-left: 4rpx;
	}
	...
	.gallery .inequacy_wrap .inequacy {
		width: 100%;
		height: 100%;
		position: absolute;
  ...
  line-height: 48rpx; */
	...
	.product-nums .border-top {
		margin-left: 24rpx;
		border-top: 1rpx #e4e4e4 solid;
		border-width: 1rpx;
	}

	.product-nums .border-top.no-border {
		border: 0;
	}

	.product-nums .nums-name {
		font-size: 24rpx;
		line-height: 84rpx;
		color: #767676;
	}

	.product-nums .nums-name .nums-name_logo {
		width: 24rpx;
		height: 24rpx;
		vertical-align: middle;
	}

	.product-nums .nums-name span {
		vertical-align: middle;
		padding-left: 10rpx;
	}

	.product-nums .nums-item-right {
		text-align: left;
	}

	.product-nums .nums-item-right .nums-item_go {
		position: absolute;
		top: 50%;
		right: 54rpx;
		transform: translateY(-50%);
	...
	.team-coupon-text-none {
		color: #eb3c39 !important;
	}

	.product-nums .nums-item-right span {
		font-size: 24rpx;
		line-height: 78rpx;
		color: #767676;
		margin-right: 14rpx;
	...
	.product-nums .nums-item-right span .item-icon_exeimg {
		height: 40rpx;
		line-height: 40rpx;
		padding: 0 10rpx;
		min-width: 88rpx;
	...
	.product-nums .nums-item-right .nums-item-right_go {
		position: absolute;
		top: 50%;
		right: 24rpx;
		transform: translateY(-50%);
	...
	.product-nums .nums-item-right .item-left-exeimg img {
		width: 110rpx;
		height: 40rpx;
		margin-right: 10rpx;
	}
	...
	.product-nums-sku .nums-item-right {
		font-size: 24rpx;
		line-height: 80rpx;
		color: #767676;
	}
  ...
  padding-bottom: calc(100rpx + env(safe-area-inset-bottom)); */
	...
	.intro .wxParse-ul .wxParse-li {
		padding-bottom: 40rpx;
	}

	.intro .wxParse-ul .wxParse-li .wxParse-h3 {
		font-size: 28rpx;
		line-height: 40rpx;
		color: #030303;
		font-weight: 400;
	...
	.intro .wxParse-ul .wxParse-li .wxParse-p {
		font-size: 24rpx;
		line-height: 35rpx;
		padding-top: 4rpx;
		color: #767676;
	...
	.footer-return {
		padding: 40rpx 0 150rpx 0;
		font-size: 0;
	}

	.footer-return .gohome {
		display: inline-block;
		position: relative;
		font-size: 28rpx;
		padding-right: 26rpx;
	...
	.footer-return .gohome .icon-right-arrow {
		right: 0;
	}
	...
	.fw_manu_box .icon-btn_qiapianguanbianniux {
		position: absolute;
		left: 50%;
	...
	.fw_manu_box .manu-fr:last-child {
		padding-bottom: 0;
	}
	...
	.drawer_attr_content .buy_now_btn {
		width: 100%;
		height: 96rpx;
		line-height: 96rpx;
		background-color: #ffe150;
	...
	.product-SKU .product-price .small {
		color: #b5b5b5;
		font-size: 18rpx;
		margin-left: 10rpx;
		text-decoration: line-through;
	...
	.attr-amount {
		...
		display: flex;
		padding-top: 40rpx;
		margin: 10rpx 30rpx 140rpx 30rpx;
		/* border-top: 1rpx #e4e4e4 solid; */
	}
	...
	.attr_name {
		...
		/* border: 1rpx #e4e4e4 solid; */
	}

	.attr_name.active {
		/* border: 1rpx #ffe150 solid; */
		color: #555;
		background-color: #ffe150;
	}

	.attr_name.disable,
	.attr_name.active.disable {
	...
	.product-sctite-box .product-sctite-box_name {
		display: block;
		width: 70%;
		font-size: 32rpx;
		line-height: 32rpx;
	...
	.product-sctite-box .product-sctite-box_right {
		position: relative;
		font-size: 26rpx;
		line-height: 32rpx;
		color: #767676;
	...
	.product-sctite-box .product-sctite-box_right span {
		margin-right: 24rpx;
	}

	.product-sctite-box .recommended-content {
		display: block;
		padding-top: 24rpx;
		padding-right: 0;
		overflow: hidden;
	...
	.product-sctite-box .recommended-content .content-ul {
		width: 100%;
		white-space: nowrap;
		overflow: hidden;
	...
	.product-sctite-box .content-ul .content-li {
		width: 226rpx;
		position: relative;
		height: 300rpx;
		margin-right: 4rpx;
	...
	.product-sctite-box .content-ul .content-li image {
		width: 112rpx;
		vertical-align: inherit;
	}

	.product-sctite-box .content-ul .content-li.li-noright {
		margin-right: 0;
		width: 112rpx;
	}

	.product-sctite-box .content-ul .recommended-box {
		position: relative;
		padding-top: 100%;
		background: #fff;
		box-sizing: border-box;
	...
	.product-sctite-box .recommended-box .recommended-img {
		position: absolute;
		top: 0;
		left: 0;
		width: 226rpx;
	...
	.product-sctite-box .content-ul .recommended-name {
		font-size: 24rpx;
		overflow: hidden;
		width: 206rpx;
		min-height: 32rpx;
	...
	.product-sctite-box .content-ul .recommended-white {
		color: #fff;
	}
	...
	.product-review-box .review-box_header {
		position: relative;
		padding: 30rpx 24rpx;
	...
	.product-review-box .review-box_header span {
		font-size: 32rpx;
		line-height: 32rpx;
		color: #030303;
	}

	.product-review-box .review-box_header i {
		font-style: normal;
		font-size: 28rpx;
		line-height: 28rpx;
		color: #767676;
	...
	.product-review-box .review-box_null {
		position: relative;
		padding: 30rpx 24rpx;
	}

	.product-review-box .review-box_null span {
		font-size: 32rpx;
		line-height: 32rpx;
		color: #030303;
	}

	.product-review-box .review-box_bottom {
		position: relative;
		display: block;
		height: 94rpx;
		color: #767676;
		text-align: center;
		border-top: 1rpx #e9e9e9 solid;
		border-width: 1rpx;
		box-sizing: border-box;
	}

	.product-review-box .review-box_bottom i {
		font-size: 28rpx;
		line-height: 94rpx;
		vertical-align: middle;
	}

	.product-review-box .review-box_bottom image {
		width: 14rpx;
		height: 28rpx;
		margin-left: 10rpx;
	}
	...
	.app-container .productLis-header {
		background: #fff;
		overflow: hidden;
		font-size: 39rpx;
		position: relative;
	...
	.productLis .theme-list {
		float: left;
		width: 100%;
		background: #fff;
	}

	.productLis .theme-list .list-li {
		position: relative;
		float: left;
		width: 50%;
		max-width: 50%;
	...
	.productLis .theme-list .list-li .menu-img {
		padding-top: 100%;
		background: #f8f8f8;
	...
	.productLis .theme-list .list-li .menu-img span {
		top: 0;
		left: 0;
		position: absolute;
		width: 100%;
	...
	.productLis .theme-list .list-li:last-child .menu-img {
		border: 0;
	}

	.productLis .theme-list .list-li .menu-text {
		color: #030303;
		font-size: 28rpx;
		padding: 20rpx 9% 0 28rpx;
		line-height: 38rpx;
		margin-bottom: 10rpx;
		display: -webkit-box;
		-webkit-box-orient: vertical;
	...
	.productLis .theme-list .list-li .menu-xl {
		position: absolute;
		bottom: 62rpx;
		right: 24rpx;
		font-size: 24rpx;
	...
	.productLis .theme-list .list-li .menu-is-svip .menu-suprice {
		padding-left: 28rpx;
		color: #767676;
		font-size: 28rpx;
		line-height: 1.3;
	...
	.productLis .theme-list .list-li .menu-is-svip .menu-suprice.color-r {
		color: #030303;
	}

	.productLis .theme-list .list-li .menu-is-svip .menu-suprice .suprice-price .n {
		font-size: 26rpx;
		letter-spacing: 1rpx;
		white-space: nowrap;
		margin-right: 4rpx;
	...
	.productLis .theme-list .list-li .menu-is-svip .menu-suprice .r {
		font-weight: 400;
		font-size: 26rpx;
		white-space: nowrap;
	}

	.productLis .theme-list .list-li .menu-is-svip .menu-suprice .p {
		margin-left: 2rpx;
		font-size: 28rpx;
		font-weight: 300;
		white-space: nowrap;
	...
	.productLis .theme-list .list-li .menu-is-svip .menu-price {
		padding-left: 28rpx;
		color: #030303;
		font-size: 28rpx;
		line-height: 1.3;
	...
	.productLis .theme-list .list-li .menu-is-svip .menu-price.color-r .n {
		color: #767676;
	}

	.productLis .theme-list .list-li .menu-is-svip .menu-price .n {
		font-size: 26rpx;
		white-space: nowrap;
		color: #030303;
	}

	.productLis .theme-list .list-li .menu-is-svip .menu-price .r {
		font-weight: 400;
		color: #f23030;
		font-size: 26rpx;
		margin-left: 2rpx;
	...
	.productLis .theme-list .list-li .menu-is-svip .menu-price .p {
		font-weight: 300;
		color: #f23030;
		font-size: 28rpx;
		margin-left: 2rpx;
	...
	.share-get-cashgift {
		...
		background-image: url("http://img.mall.surex.cn/cilicili_icons/share-cashgift-bg.png");
		background-size: 100% auto;
		margin: 20rpx 0;
	}

	.share-get-cashgift:before {
		content: "";
		position: absolute;
		top: -20rpx;
	...
	.share-get-cashgift .cashgift-title {
		text-align: center;
		font-size: 28rpx;
		padding: 28rpx 0 20rpx 0;
	}

	.share-get-cashgift .cashgift-ticket {
		padding: 0 30rpx 34rpx 30rpx;
	}

	.share-get-cashgift .cashgift-ticket .ticket-inner {
		height: 160rpx;
		background-image: url("http://img.mall.surex.cn/cilicili_icons/ticket-bg.png");
		background-size: 100% 100%;
		position: relative;
	}

	.share-get-cashgift .cashgift-ticket .ticket-price {
	...
	.share-get-cashgift .cashgift-ticket .ticket-price i {
		font-size: 50rpx;
	}

	.share-get-cashgift .cashgift-ticket .ticket-price.num3 {
		font-size: 64rpx;
	}

	.share-get-cashgift .cashgift-ticket .ticket-price.num4 {
		font-size: 58rpx;
	}

	.share-get-cashgift .cashgift-ticket .ticket-price.num3 i,
	.share-get-cashgift .cashgift-ticket .ticket-price.num4 i {
		font-size: 32rpx;
	}

	.share-get-cashgift .cashgift-ticket .ticket-content {
		position: relative;
		padding-left: 29.7%;
	...
	.share-get-cashgift .cashgift-ticket .ticket-title {
		font-size: 28rpx;
		padding-bottom: 8rpx;
		white-space: nowrap;
		overflow: hidden;
	...
	.share-get-cashgift .cashgift-ticket .ticket-desc {
		font-size: 20rpx;
		color: #767676;
		white-space: nowrap;
		overflow: hidden;
	...
	.share-get-cashgift .cashgift-ticket .ticket-button,
	.ticket-button wx-button {
		width: 120rpx;
		height: 60rpx;
		background-color: #ffcd23;
		font-size: 28rpx;
	...
	.share-get-cashgift .cashgift-ticket .ticket-button wx-button::after {
		opacity: 0;
	}
	...
	.share-get-cashgift .cashgift-ticket .ticket-button.disable {
		background-color: #e9ebec;
		color: #bbb;
	}

	/*share-get-cashgift-modal*/
	...
	.share-get-cashgift-modal .modal-content {
		position: absolute;
		top: 45%;
		-webkit-transform: translateY(-50%);
		transform: translateY(-50%);
	...
	.share-get-cashgift-modal .modal-content .content-box {
		background-color: #fff;
		border-radius: 10rpx;
	}

	.share-get-cashgift-modal .modal-content .content-box .box-top {
		height: 300rpx;
		background-image: url("http://img.mall.surex.cn/cilicili_icons/share-get-cashgift-modal.png");
		background-size: 100% 100%;
	}

	.share-get-cashgift-modal .modal-content .content-box .box-top.success {
		background-image: url("http://img.mall.surex.cn/cilicili_icons/share-get-cashgift-modal-success.png");
	}

	.share-get-cashgift-modal .modal-content .content-box .box-title {
		color: #030303;
		text-align: center;
		font-size: 40rpx;
		padding-top: 48rpx;
	...
	.share-get-cashgift-modal .modal-content .content-box .box-text {
		color: #030303;
		text-align: center;
		font-size: 32rpx;
	...
	.share-get-cashgift-modal .modal-content .content-box .box-text span {
		color: #f9ae08;
	}

	.share-get-cashgift-modal .modal-content .content-box .box-button {
		padding: 0 32rpx 32rpx 32rpx;
	}

	.share-get-cashgift-modal .modal-content .content-box .box-button .button-inner {
		background-color: #ffcd23;
		text-align: center;
		font-size: 36rpx;
		color: #71400b;
	...
	.share-get-cashgift-modal .modal-content .content-close {
		position: absolute;
		left: 50%;
		-webkit-transform: translateX(-50%);
		transform: translateX(-50%);
		bottom: -160rpx;
		background-image: url("http://img.mall.surex.cn/cilicili_icons/share-get-cashgift-close.png");
		background-size: 100% 100%;
		width: 80rpx;
		height: 80rpx;
	}

	/*.share-get-cashgift-modal.modal-wx*/

	.share-get-cashgift-modal.modal-wx .modal-content {
		top: 0;
		padding: 0;
	...
	.share-get-cashgift-modal.modal-wx .modal-content .wx-share-promption {
		width: 100%;
		padding: 20rpx 64rpx 0 114rpx;
		box-sizing: border-box;
	}

	.share-get-cashgift-modal.active {
		display: block;
	}

	.share-get-cashgift-modal.notactive {
		display: none;
	}

	/* 分享按钮 */

	.share-get-cashgift-modal.modal-wx .icon-content {
		position: absolute;
		z-index: 1001;
		width: 100%;
		box-sizing: border-box;
	...
	.share-get-cashgift-modal.modal-wx .icon-content .share-content {
		background-color: #fff;
		box-sizing: border-box;
	}

	.modal-wx .icon-content .share-content .share-title {
		font-size: 32rpx;
		color: #000;
	...
	.modal-wx .icon-content .share-content .share-cancel {
		font-size: 32rpx;
		color: #fff;
		padding: 24rpx;
		text-align: center;
	...
	.modal-wx .icon-content .share-content .share-icon {
		padding: 30rpx 24rpx;
		justify-content: center;
	}

	.modal-wx .icon-content .share-content .share-icon .big-icon {
		background-color: transparent;
		padding: 0;
		margin: 0 40rpx;
		font-size: 100rpx;
	...
	.modal-wx .icon-content .share-content .share-icon .icon-text {
		font-size: 24rpx;
		color: #555;
		text-align: center;
		padding: 10rpx 0;
	...
	.modal-inner .modal-inner-header {
		font-size: 32rpx;
		line-height: 32rpx;
	...
	.modal-inner .modal-coupon-title {
		font-size: 32rpx;
		line-height: 32rpx;
	...
	.modal-inner .modal-coupon-close {
		position: absolute;
		right: 20rpx;
		top: 20rpx;
	}

	.modal-inner .modal-coupon-close image {
		width: 30rpx;
		height: 30rpx;
	}

	.modal-inner .modal-inner-main {
		max-height: 700rpx;
	}

	.modal-inner .modal-inner-main .modal-coupon-box {
		padding: 0 24rpx;
		padding-bottom: 168rpx;
	}

	.modal-inner .modal-inner-main .modal-inner-block {
	...
	.modul-review-manu.no-border {
		border: 0;
	}

	.modul-review-manu .review-manu_header {
		position: relative;
		padding-bottom: 20rpx;
		background-image: url("http://img.mall.surex.cn/cilicili_icons/default-avatar.png");
		background-repeat: no-repeat;
		background-size: 68rpx 68rpx;
	}

	.modul-review-manu .review-manu_header image {
		border-radius: 50%;
		width: 68rpx;
		height: 68rpx;
	}
	...
	.modul-review-manu .review-manu_header .header-user_name {
		font-size: 28rpx;
		line-height: 36rpx;
		width: 250rpx;
		height: 25rpx;
	...
	.modul-review-manu .review-manu_header .header-user_name span {
		overflow: hidden;
		text-overflow: ellipsis;
		display: -webkit-box;
		-webkit-line-clamp: 1;
	...
	.modul-review-manu .review-manu_main .manu_main_text {
	...
	.modul-review-manu .manu_main_text .limit-words {
		display: block;
		width: 100%;
		font-size: 24rpx;
		line-height: 30rpx;
	...
	.modul-review-manu .manu_main_text .limit-words image {
		display: inline-block;
		margin-left: 6rpx;
		width: 30rpx;
		height: 18rpx;
	...
	.modul-review-manu .manu_main_text .limit-words .show {
	...
	.modul-review-manu .manu_main_text .limit-words .hidden {
	...
	.modul-review-manu .review-manu_main .manu_main_imgs {
		position: relative;
		display: inline-block;
	...
	.modul-review-manu .review-manu_main .manu_main_imgs span {
		position: absolute;
		top: 0;
		left: 0;
		width: 100%;
	...
	.modul-review-manu .review-manu_reply {
		position: relative;
		padding: 24rpx 30rpx;
		background: #f2f2f2;
		margin-bottom: 30rpx;
	...
	.modul-review-manu .review-manu_reply:after {
		content: '';
		position: absolute;
		top: -10rpx;
		left: 34rpx;
	...
	.modul-review-manu .review-manu_reply i {
		font-size: 28rpx;
		line-height: 40rpx;
		color: #767676;
	}

	.modul-review-manu .review-manu_additional {
		padding: 16rpx 0;
	}

	.modul-review-manu .review-manu_additional .manu_additional_header {
		font-size: 28rpx;
		line-height: 28rpx;
		color: #22b32a;
	...
	.modul-review-manu .review-manu_additional .manu_additional_text {
		font-size: 28rpx;
		line-height: 40rpx;
		color: #030303;
		padding-bottom: 14rpx;
	...
	.modul-review-manu .review-manu_additional .manu_main_imgs {
		position: relative;
		display: inline-block;
		display: -webkit-inline-flex;
		padding-top: 226rpx;
	...
	.modul-review-manu .review-manu_additional .manu_main_imgs span {
		position: absolute;
		top: 0;
		left: 0;
		width: 100%;
	...
	.modul-review-manu .reviw-manu_attrs {
		font-size: 24rpx;
		line-height: 24rpx;
		color: #bbb;
		padding-top: 14rpx;
	...
	.menu-share .shareMenu_col {
		width: 50%;
		text-align: center;
	}

	.menu-share .shareMenu_col button {
		height: 200rpx;
		line-height: 2.5;
	}

	.menu-share .shareMenu_col image {
		width: 100rpx;
		height: 100rpx;
	}

	.menu-share .shareMenu_col .share-text {
		font-size: 26rpx;
		color: #979797;
	}
	...
	.share-modal-pic .modal-content .share-img {
		position: relative;
		width: 566rpx;
		height: 960rpx;
		background-color: #fff;
	...
	.share-modal-pic .modal-content .share-img canvas {
		position: absolute;
		top: 0;
		left: 1600rpx;
	}

	.share-modal-pic .modal-content .share-img .show-img {
		width: 100%;
		height: 100%;
	}

	.share-modal-pic .modal-content .share-btns {
		justify-content: center;
		margin-top: 64rpx;
	}

	.share-modal-pic .modal-content .share-btns .share-button {
		width: 272rpx;
		height: 80rpx;
		background-color: #fff;
		border-radius: 12rpx;
	...
	.share-modal-pic .modal-content .share-btns .share-button.btn-right {
		background-color: #ffe150;
		color: #555;
		margin-left: 22rpx;
	}
	...
	.fw_manu_box .zj-erweima {
		width: 100%;
		padding-top: 40rpx;
		text-align: center;
	}

	.fw_manu_box .zj-erweima image {
		height: 200rpx;
		width: 200rpx;
	}
	...
	.rent_info .rent_tab .active_item {
		background-color: #eb3c39;
		color: #fff;
	}

	.rent_info .rent_table {
		padding: 0 24rpx;
	}

	.rent_info .rent_table .row {
		height: 80rpx;
		line-height: 80rpx;
		border-bottom: 1rpx solid #ddd;
	}

	.rent_info .rent_table .row:last-child {
		border-bottom: none;
	}

	.rent_info .rent_table .left {
		flex: 1;
		text-align: left;
	}

	.rent_info .rent_table .right {
		flex: 1;
		text-align: right;
	}
	...
	.product-form-bottom.home {
		right: 635rpx;
	}

	.product-form-bottom.cart {
		right: 446rpx;
	}

	.product-form-bottom button {
		width: 100rpx;
		height: 98rpx;
		background-color: transparent;
		color: transparent;
	...
	.team-bar {
		position: relative;
		background: linear-gradient(90deg, rgba(255, 234, 134, 1) 0%, rgba(255, 225, 80, 1) 100%);
	...
	.team-bar .team-price .team-price-i {
		font-size: 24rpx;
		line-height: 56rpx;
		color: #303030;
		font-weight: 500;
	...
	.team-bar .team-price .team-price-p {
		font-size: 56rpx;
		color: #303030;
		font-weight: 700;
	}

	.team-bar .team-price .team-price-n {
		font-size: 24rpx;
		color: #303030;
		line-height: 56rpx;
		margin-left: 14rpx;
	...
	.team-bar .team-share image {
		width: 28rpx;
		height: 28rpx;
	}

	.team-bar .team-share text {
		padding-left: 10rpx;
		color: #555;
		font-size: 24rpx;
	}
	...
	.join-team .join-header .join-title {
		font-size: 28rpx;
		color: #333;
		font-weight: 500;
	}

	.join-team .join-header .join-more {
		font-size: 24rpx;
		color: #aaa;
		font-weight: 500;
	}
	...
	.tips-active {
		border-radius: 16rpx;
		border: 2rpx solid #ff7028 !important;
	...
	.getPhoneModal .btn-footer button {
		border-radius: 0;
	}

	.getPhoneModal .btn-footer .close-phone-btn {
		width: 50%;
		height: 72rpx;
		display: flex;
		align-items: center;
	...
	.getPhoneModal .btn-footer .get-phone-btn {
		width: 50%;
		height: 72rpx;
		display: flex;
		align-items: center;
