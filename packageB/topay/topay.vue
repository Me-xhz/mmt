		...
		<view>
			<view class="pay-body">
				...
				<view class="set-money">
					...
					<input class="toSetPrice" type="digit" :maxlength="priceMaxLength" @blur="bindPriceBlur" @focus="bindPriceFocus"
					 @input="bindPriceInput" focus="true" :value="price"></input>
				</view>
				<view class="hr">
			...
			<view class="show-money">
				<view v-if="show_price">
					...
					<view class="price-box">
						...
						<text class="price-font font-14 price-discount" v-if="show_discount">(小程序支付优享{{discount*10}}折)</text>
					</view>
				</view>
			</view>
			<view class="to-pay" :animation="animationView">
				<button :class="(check_price==true ? 'pay-btn' : 'no-pay-btn')" @tap="toPay" :data-payprice="now_price">
					<text>立即支付</text>
				</button>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			bindPriceInput: function(e) {
				var num = e.detail.value; // this.data.priceMaxLength = -1;
				...
				if (e.detail.value >= 0) {
					...
					this.setData({
						check_price: true,
						show_price: true,
						now_price: Math.floor(num * this.discount * 100) / 100
					});
				} else {
					this.setData({
						price: "",
			...
			toPay: function(e) {
				...
				if (payprice == 0) {
					...
					var data = {
						order_amount: that.now_price * 100
					};
					common.ApiGateWayTest(uri, data, true, function(res) {
						var data = res.data;
