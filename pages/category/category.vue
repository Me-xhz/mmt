...
<view v-if="showPage">
  ...
  <view class="section row">
    ...
    <scroll-view scroll-y="true" class="left">
      <view v-for="(item, index) in types" :key="index" :data-item="item" @tap="tapType" :class="(item.cat_id == currType ? 'actived t5' : '')">
        {{item.name}}
      </view>
    </scroll-view>
    ...
    <view class="right">
      <view class="tab_bar">
        <view :class="'tab_item ' + (sort==0?'change_bar':'')" data-sort="0" @tap="sortTypes">全部</view>
        <view :class="'tab_item ' + (sort==1?'change_bar':'')" data-sort="1" @tap="sortTypes">最新</view>
        <view :class="'tab_item row ' + (sort==2?'change_bar':'')" data-sort="2" @tap="sortTypes">
          <text>价格</text>
          <image v-if="order == 1" class="icon-paixu" :src="iconURL + '/icon-class-up01.png'"></image>
          <image v-else class="icon-paixu" :src="iconURL + '/icon-class-down01.png'"></image>
        </view>
        <view class="ac_bb" v-if="!typeTree.ad.ad_code"></view>
      </view>
      <scroll-view class="content" scroll-y="true" @scrolltolower="onReachBottom" @scroll="scrollEvent">
        <view class="ac_img" @tap="goWebView" :data-item="typeTree.ad">
          <image :src="typeTree.ad.ad_code" v-if="typeTree.ad.ad_code"></image>

        </view>
        <view class="typeTree">
          <view class="good_list">
              <view class="good" v-for="(item, index) in good_list" :key="index">
                <!-- <navigator hover-class="none" url='/pages/product/product?entryType=external&productId={{item.goods_id}}'> -->
                  <view class="good_pic" :style="'background-image:url(' + iconURL + '/cstp.png)'">
                    ...
                    <view class="discount_label" v-if="item.img_content&&item.img_content.length>0">{{item.img_content}}</view>
                ...
                <view class="good_info">
                  <!-- <navigator hover-class="none" url='/pages/product/product?entryType=external&productId={{item.goods_id}}'> -->
                    ...
                    <view class="price">
                      ...
                      <text v-if="item.shop_type == 0 && item.market_price != item.shop_price" class="old_price">￥{{item.market_price}}</text>
                ...
                <form @submit="goToProduct" class="category-from-goods" :data-goodsId="item.goods_id" report-submit="true">
