...
<view v-for="(modelGroup, index) in indexModel" :key="index">
  <view v-for="(item, index2) in modelGroup" :key="index2">
    <view class="ad" v-if="item.type==2">
      <image class="ad_img" :src="item.title" mode="widthFix"></image>
    </view>
    <view class="ad" v-if="item.type==1&&item.route.url!='coupon'" @tap="goWebView" :data-param="item.route.param">
      <image class="ad_img" :src="item.pic" mode="widthFix"></image>
    </view>
    <!-- 优惠券 -->
    <view class="coupon_wrap" v-if="item.type==1&&item.route.url=='coupon'">
      <image class="cover_img" :src="iconURL + '/img_ztyhqzz@2x.png'"></image>
      <scroll-view scroll-x="true" class="scroll_wrap">
        <view class="coupon" :style="'background-image:url(' + iconURL + '/img_xrzxyhqbg@2x.png)'" v-for="(coupon, index3) in item.route.param" :key="index3">
          <view class="price">
      ...
      <view class="total_box">
        ...
        <view class="get_btn" v-if="item.route.take==1" @click.stop="akeyToGet" :data-ad_id="item.route.ad_id" :data-param="item.route.param" :style="'background-image:url(' + iconURL + '/btn_yjlq@2x.png)'"></view>
    ...
    <!-- 新人商品 -->
    <view class="ac_new_person good_list_wrap" v-if="item.type==3&&item.goodscss==1" :style="'background-image:url(' + iconURL + '/home-newlyweds2@2x.png)'">
      <!-- <view class='iconfont icon-btn_xianshifengqiangguizex'></view> -->
      <view class="good_list">
        <view :class="'good_box ' + (index!=2?'mr6':'')" v-for="(good, index3) in item.goods" :key="index3">
          <view class="good_info" @click.stop="goToProduct" :data-goodsId="good.goods_id">
            <view class="good_pic inequacy_wrap">
              <image class="goods_lable" v-if="good.goods_lable.length>0" :src="good.goods_lable"></image>
              <view class="discount_label" v-if="good.img_content&&good.img_content.length>0 && type != 3">{{good.img_content}}</view>
              <image :src="good.list_img"></image>
              <view class="inequacy" v-if="good.is_enough==0">
          ...
          <view class="add_cart">
            <form @submit="addShopCart" v-if="good.buy_now == 0" data-buyNow="0" data-type="bottomcart" :data-productId="good.goods_id" report-submit="true">
              <button class="addToShopCart" formType="submit">
                <text>加入购物车 <text class="iconfont icon-btn_shangxianggouwuchex"></text></text>
              </button>
            </form>
            <form @submit="nowPurchase" v-if="good.buy_now == 1" data-buyNow="1" data-type="bottomcart" :data-productId="good.goods_id" report-submit="true">
              <button class="addToShopCart" formType="submit">
    ...
    <!-- 普通商品 -->
    <view v-if="item.type==3&&item.goodscss==2">
      <view class="common_good good_list_wrap" v-for="(row, index3) in item.goods" :key="index3">
        <view class="good_list" style="padding-bottom:12rpx">
          <view :class="'good_box ' + (index!=2?'mr6':'')" v-for="(good, index4) in row" :key="index4">
            <view class="good_info" @click.stop="goToProduct" :data-goodsId="good.goods_id">
              <view class="good_pic inequacy_wrap">
                <image class="goods_lable" v-if="good.goods_lable.length>0" :src="good.goods_lable"></image>
                <view class="discount_label" v-if="good.img_content&&good.img_content.length>0 && type != 3">{{good.img_content}}</view>
...
<view class="iconfont icon-btn_shangxianggouwuchex toCart" @click.stop="toCart"></view>

<!--SKU弹窗-->
<view class="drawer_screen" @click.stop="setModalStatus" v-if="showModalStatus"></view>
<view :animation="animationData" :style="'opacity:1;z-index:' + (itemData.buy_now==1?'2000':'1200') + ';'" class="drawer_attr_box" v-if="showModalStatus">
  ...
  <view class="drawer_attr_content">
    ...
    <form v-if="buy_now == 0" @submit="addShopCart" data-type="bottomcart" :data-productId="productid" report-submit="true">
      <button type="primary" class="buy_now_btn" :data-productId="productid" formType="submit" data-type="bottomcart">加入购物车</button>
    </form>
    <form v-if="buy_now == 1" @submit="nowPurchase" data-type="bottomcart" :data-productId="productid" report-submit="true">
