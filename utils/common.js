	...
	return new Promise(function(resolve, reject) {
		...
		wx.login({
			success: function(res) {
				...
				console.log('获取登录信息2',data)
				ApiGateWayTest(userUri, data, '', function(res) {
					console.log('获取登录信息2',res)
					var data = res.data;
...
function register(callback, userData) {
	...
	wx.login({
		success: function(res) {
			...
			var wxmUri = '/user/thirdLogin?oauth=wxm'; // 如果缓存有分享人的user_id,一并传参
...
function checkLogin(e, callback) {
	...
	wx.login({
		success: function(res) {
			var checkLoginUri = '/user/checkLogin'; // Mobiledata.code = res.code;

			var checkLoginData = {
				oauthCode: res.code // 检测登录API

			};
			ApiGateWayTest(checkLoginUri, checkLoginData, '', function(data) {
				if (data.data.success == 1) {
					...
					if (data.status == 0) {
						...
						// 新用户注册
						// 获取手机号
						var mobileUri = '/user/getWXMobile'; // 重新获取code获取手机号
						...
						wx.login({
							success: function(res) {
								...
								ApiGateWayTest(mobileUri, Mobiledata, '', function(d) {
									if (d.data.success == 1) {
										var mobile = d.data.result.mobile; // 第三方注册
...
function getpickup(callback) {
	...
	return new Promise(function(resolve, reject) {
		getLocation().then(function(ati) {
			...
			console.log('atiatiati', ati)
			ApiGateWayTest(uri, {
				atitude: ati
			}, false, function(res) {
				console.log('resresres', res.data.result.cities);
...
function goAdWebView(e) {
	...
	formIdUpdate(e);
	console.log('item.media_type item.media_type', item.media_type);
...
module.exports = {
	...
	goRegister2:goRegister2,
	wxEnvVersionFun: wxEnvVersionFun,
