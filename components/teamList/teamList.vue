...
<swiper :class="'team-list ' + (teamList.length==1?'list-height':'')" autoplay="true" interval="5000" vertical="true">
    <swiper-item class="team-list-item" v-for="(item, index) in teamList" :key="index" v-if="index%2==0">
        <view :class="'join-list ' + (teamList.length==1?'bottom-none':'')">
            ...
            <view class="join-list-right">
                ...
                <form report-submit="true" @submit="toAddTeam" data-alpha-beta="0" :data-found_id="teamList[index].found_id" :data-team_id="teamList[index].team_id">
        ...
        <view class="join-list bottom-none" v-if="index<teamList.length-1">
            ...
            <view class="join-list-right">
                ...
                <form report-submit="true" @submit="toAddTeam" data-alpha-beta="0" :data-found_id="teamList[index+1].found_id" :data-team_id="teamList[index+1].team_id">
                    <button form-type="submit" class="join-list-btn">
                        去拼团
                    </button>
                </form>
...
export default {
  ...
  methods: {
    ...
    countDown(endtime) {
      ...
      that.teamList.forEach((val, index) => {
        ...
        timeout[index] = setInterval(() => {
          ...
          if (lesstime > 0) {
            ...
            if (index + 1 == that.teamList.length) {
              that.setData({
    ...
    toAddTeam(e) {
      ...
      this.team_id = e.currentTarget.dataset.team_id;
      common.formIdUpdate(e);
      wx.navigateTo({
        url: '/packageA/groupbuy/groupbuy?pickup_id=' + that.teamFound.pickup_id + '&found_id=' + fond_id
    ...
    getGroupInfo(found_id) {
      var that = this;
      var uri = getApp().globalData.getPath.teamFound;
      var data = {
        found_id: found_id
      };
    ...
    toAdd(msg) {
      ...
      if (!getApp().globalData.checkSessionKey || isOldUser == 2) {
        common.goRegister();
        return false;
      }

      var checkOrderUri = getApp().globalData.getPath.preCheckOrder;
      var orderProduct = [{
      ...
      var checkOrderData = {
        ...
        time_atitude: getApp().globalData.dataBase.userAtitude
      };
